...
// PSV
    ...
    var afkLimit = 12000; // limite de afk (12 segundos)
...
function handleInactivity() {
    ...
    if (currentTime - lastInactivityCheck < 5000) {
        return;
    }
    ...
    players.forEach(player => {
        if (!getAFK(player)) {
            ...
            if (activity === null || activity === 0) {
                setActivity(player, currentTime);
                return;
            }
            
            if (currentTime - activity >= afkLimit) {
                console.log(`AFK detectado para ${player.name}`);
                setAFK(player, true);
                // Guardar rol original antes de asignar AFK
    ...
    if (afkDetected && isMatchInProgress()) {
        const specs = players.filter(p => p.team === 0 && !getAFK(p));
        
        if (specs.length === 0) {
            room.sendChat("⚠️ No hay jugadores disponibles para reemplazar al AFK. Forzando 2v2...");
            loadMap(classicMap, scoreLimitPractice, timeLimitPractice);
            balanceTeams();
        } else {
            balanceTeams();
...
function EquiposEsportsFun(player) { // !esports
    ...
    function checkTime() {
        ...
        // Variable para controlar si ya se anunció el fin del partido
        if (!game.endGameAnnounced) {
            if (Math.abs(scores.time - scores.timeLimit) <= 0.01 && scores.timeLimit != 0) {
                if (scores.red != scores.blue) {
                    if (checkTimeVariable == false) {
                        checkTimeVariable = true;
                        game.endGameAnnounced = true; // Marcar que ya se anunció
                        setTimeout(() => {
        ...
        // Modificar la lógica del tiempo extra
