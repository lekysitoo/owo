...
// setupAutoStart();
    ...
    function getChatFormat(player) {
        let stats = JSON.parse(localStorage.getItem(getAuth(player))) || { GL: 0 };
        let rankInfo = getRankInfo(stats.GL);
        let role = getRole(player);
        let prefix = "";
        let chatColor = rankInfo.chatColor;
    ...
    function endGame(winner) {
        ...
        scores.red == 0
            ? scores.blue == 0
                ...
                : room.sendAnnouncement('🏆 ' + GKList[1].name + ' kept a CS ! ')
            : scores.blue == 0
            ? room.sendAnnouncement('🏆 ' + GKList[0].name + ' kept a CS ! ')
            : null;
...
function updateStats() {
    ...
    setInterval(() => {
        ...
        if (statNumber % 5 == 0) {
            ...
            room.sendAnnouncement("Matches Played> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1]);
        }
        if (statNumber % 5 == 1) {
            Object.keys(localStorage).forEach(function(key) {
                if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].includes(key)) {
            ...
            room.sendAnnouncement("Victories> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1]);
        }
        if (statNumber % 5 == 2) {
            Object.keys(localStorage).forEach(function(key) {
                if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].includes(key)) {
        ...
        if (statNumber % 5 == 3) {
            ...
            room.sendAnnouncement("Assistance> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1]);
        }
        if (statNumber % 5 == 4) {
            Object.keys(localStorage).forEach(function(key) {
                if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].includes(key)) {
...
if (message.length > 1 && message[0].toLowerCase() == 't' && message[1] == ' ') {
    ...
    if (player.team == Team.RED) {
        ...
        room.getPlayerList().forEach((element) => {
            if (element.team == Team.RED || (typeof spyingAdmins !== 'undefined' && spyingAdmins.has(element.id))) {
                ...
                room.sendAnnouncement(
                    `${spyPrefix}🔴 [EQUIPO ROJO] ${prefix}${player.name}: ${message.substr(2)}`, 
                    element.id, 
        ...
        room.getPlayerList().forEach((element) => {
            if (element.team == Team.BLUE || (typeof spyingAdmins !== 'undefined' && spyingAdmins.has(element.id))) {
                ...
                room.sendAnnouncement(
                    `${spyPrefix}🔵 [EQUIPO AZUL] ${prefix}${player.name}: ${message.substr(2)}`, 
                    element.id, 
                    isSpy ? 0xFF4C4C : chatColor, 
                    "bold", 
                    0
        ...
        room.getPlayerList().forEach((element) => {
            if (element.team == Team.SPECTATORS || (typeof spyingAdmins !== 'undefined' && spyingAdmins.has(element.id))) {
                ...
                room.sendAnnouncement(
                    `${spyPrefix}👥 [ESPECTADORES] ${prefix}${player.name}: ${message.substr(2)}`, 
                    element.id, 
                    isSpy ? 0xFF4C4C : chatColor, 
                    "bold", 
                    0
    ...
    room.onGameStart = function(byPlayer) {
        ...
        if (players.length === 1) {
            ...
            room.sendAnnouncement(centerText("!bra - Uniforme Brasil | !ger - Uniforme Alemania"), null, 0x2EF55D);
        }
