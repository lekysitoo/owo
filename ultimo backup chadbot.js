...
function getRandomScorerMessage() {
    ...
    const scorerMessages = [
        ...
        `🔥⚽ ${scorerName} se cogió a ${equipoRivalNombre} y ${arqueroRival} no pudo hacer nada.`,
        `💥🔥🍆 ${scorerName} no para de repartir pija, se coge absolutamente a todos los de ${equipoRivalNombre}. 😈🥵`,
        `🤯💥⚡ El golazo que hizo ${scorerName}, pijudo hijo de re mil puta. ⚽🔥`,
        `👌⚽👏 ¡Bien definido por ${scorerName}!`,
        `🍷🚬🗿 La definición de ${scorerName} definitivamente es cine.`,
        `⚽🔥 ¡Golazo de ${scorerName} que está jugando desnudo! 🔞🍆`,
        `🔥⚽ Eduque ${scorerName}, eduque 👏👏`,
        `💪🔥⚽ Cuando sos crack, sos crack... ¡Y ${scorerName} lo acaba de demostrar! 👑`,
        `⚡⚽🍆 ${scorerName} se cogió a ${arqueroRival} con ese gol, alta putita el arquero de ${equipoRivalNombre}! 🧤🥵`,
        `💀⚽🔥 Alta putita ${equipoRivalNombre}, ${scorerName} se los cogió sin piedad. 🚬😎`,
        `🤩⚡🔥 ¡Naa, que golazo les marcó ${scorerName} a los de ${equipoRivalNombre}! 😱⚽`,
        `🎯⚽ ¡99 de definición, ${scorerName} definió con la punta de su tremenda pija! 🍆🔥`,
        `💥⚽🔥 ¡Ufff, qué golazo acaba de marcarle ${scorerName} a los de ${equipoRivalNombre}! 😱⚽`,
        `👀🍑🔥 ${scorerName} le rompió bien el ojete a los ${equipoRivalNombre} con ese golazo! 💥⚽`,
        `⚽👀 ¡Mirá el golazo que clavó el pijudo hijo de re mil puta de ${scorerName}! 😈🍆🔥`,
        `⚽🔥 ${scorerName} se cogió a ${arqueroRival} con ese gol. 🔥`,
        `⚽🔥 ${scorerName} le rompió bien el ojete a ${arqueroRival}. 🍑💥🔥`,
        `🔥 Parece que a los de ${equipoRivalNombre} les encanta comer pija de ${scorerName} 🍆🥵`,
        `🔥⚽ ${scorerName} le rompió bien el orto a ${equipoRivalNombre} con ese golazo. 🍑💥🥵`,
        `👅🍑🔥 Los de ${equipoRivalNombre} son la puta preferida de ${scorerName}. 😏⚽`,
        `🧤🍩 ${arqueroRival} entregó el rosquete y ${scorerName} se lo cogió con ese gol. ⚽🔥`,
        `🔥⚽ El jugador ${scorerName} no para de coger a los de ${equipoRivalNombre}. 😈`,
        `🍆💦🥵 ${scorerName} le pasó toda la pija en la cara a ${arqueroRival} con ese golazo. ⚽🔥`,
        `😈🔥⚽ Llegó ${scorerName} y ya empezó a repartir pija. 🍆🥵`,
        `👅🍑🔥 El arquero ${arqueroRival} goloso como siempre entregándole el marrón a ${scorerName}. 😏🍩`
    ];
...
function getRandomOwnGoalScorerMessage() {
    ...
    const ownGoalScorerMessages = [
        `😵‍💫 Alto pelotudo ${scorerName}, se olvidó que tenía que patear al otro arco el hijo de re mil puta. 🎯🤦‍♂️`,
        `🔥 ¡El virgo de ${scorerName} se acaba de meter un gol en contra, no puede ser tan burro, por Dios. 🫏💀`,
        `🍑💦 ${scorerName} entregó la cola con ese autogol, alto virgen el hijo de puta. 🤡🔥`,
        `🎁 ¡Alta putita ${scorerName}!, le regaló el gol a ${equipoRivalNombre} como si fuera Navidad. 🎄⚽`,
        `🕵️‍♂️ Parece infiltrado el pelotudo ${scorerName}, no puede ser tan pete. 🤦‍♂️💀`,
        `🙃 Parece que al jugador ${scorerName} le encanta entregarle el rosquete a ${equipoRivalNombre}, alta putita. 🍑🔥`,
        `⚽🤦‍♂️ ${scorerName} pensó que jugaba para ${equipoRivalNombre} y les regaló un gol. 🎭💀`,
        `😨 Se cagó todo ${scorerName} con ese gol en contra, le encanta entregar el marrón a los de ${equipoPropioNombre}. 🍩💦`,
        `🔥 Alta putita ${scorerName}, ni los de ${equipoRivalNombre} le hacen ese gol a ${equipoPropioNombre} como él lo hizo. ⚽💀`,
        `🥴 Está re drogado el hijo de puta de ${scorerName}, mirá el autogol que se hace. 💊💀`,
        `💀🔥 El jugador ${scorerName} se confundió de arco, tremendo pelotudo el jugador de ${equipoPropioNombre}. 🤦‍♂️⚽`,
        `🤣 Ni los de ${equipoRivalNombre} le meten goles a ${equipoPropioNombre} como el autogol que hizo ${scorerName}, qué hijo de puta. ⚽🔥`
    ];
...
function playerChat(player, message) {
    let msgArray = message.split(/ +/);
    if (msgArray[0].length < 2) return false;
    
    let playerID = parseInt(msgArray[0].substring(2));
    if (isNaN(playerID)) return false;
    
    let playerToSend = room.getPlayer(playerID);
    if (playerToSend == null) {
        room.sendAnnouncement(`No se ha encontrado ningún jugador con id ${playerID}!`, player.id, errorColor, "bold", HaxNotification.CHAT);
        return false;
    }
    
    let messageToSend = message.substring(message.indexOf(" ") + 1);
    
    // Enviar mensaje al destinatario
    room.sendAnnouncement(`🔒 DM de ${player.name}: ${messageToSend}`, playerToSend.id, 0xFFFF00, "bold", HaxNotification.CHAT);
    room.sendAnnouncement(`🔒 DM a ${playerToSend.name}: ${messageToSend}`, player.id, 0xFFFF00, "bold", HaxNotification.CHAT);
    
    // Enviar a espías que no son parte del DM
    let players = room.getPlayerList();
    for (let i = 0; i < players.length; i++) {
        if (spyModeEnabled.has(players[i].id) && players[i].id !== player.id && players[i].id !== playerToSend.id) {
            room.sendAnnouncement(`[SPY] [DM] ${player.name} → ${playerToSend.name}: ${messageToSend}`, players[i].id, 0x888888);
        }
    }
...
if (message.startsWith("ac ")) {
    ...
    if (msgArray[0].toLowerCase() == 't') {
     ...
     // Para mensajes privados con @@
     if (msgArray[0].length >= 3 && msgArray[0].substring(0, 2) === '@@') {
